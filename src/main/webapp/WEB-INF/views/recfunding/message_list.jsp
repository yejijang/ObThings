...
.chat_ib .time{ font-size:14px; float:right; font-family: 'Nanum Pen Script'; color:#989898; margin-left:110px;}
.chat_ib p{ font-size:14px; color:#989898; margin:auto;  padding-left:30px;}
.chat_img {
  float: left;
  width: 11%;
}
...
<script>
	...
	const MessageContentList = function(room) {
		...
		$.ajax({
			url:"message_content_list.action",
			method:"GET",
			data:{
				room : room,
				memberSeq : ${memberSeq}, 
	...
	const SendMessage = function(room, other_nick){
		...
		if(content == ""){
			...
			$.ajax({
				url:"message_send_inlist.action",
				method:"GET",
				data:{
					room : room,
